 ...
 */
function commitAndPush({ branch, dir, message, name, email, debug, }) {
    return __awaiter(this, void 0, void 0, function* () {
        const dbg = (debug !== null && debug !== void 0 ? debug : core.debug);
        dbg('Adding all files');
        yield spawn_1.spawn('git', { cwd: dir }, 'add', '.');
        dbg('Committing');
        yield spawn_1.spawn('git', { cwd: dir }, '-c', `user.name='${name}'`, '-c', `user.email='${email}'`, 'commit', '--message', message);
        dbg('Pushing');
        yield spawn_1.spawn('git', { cwd: dir }, 'push', 'origin', branch);
    });
}
...
const commitAndPush_1 = __webpack_require__(273);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { dir, branch, message, name, email } = readInputs_1.readInputs();
            const { debug } = core;
            yield commitAndPush_1.commitAndPush({ dir, branch, message, name, email, debug });
        }
        catch (error) {
